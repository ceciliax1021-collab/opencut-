...
export interface SmartCleanupConfig {
  rules: CleanupRule[];
  excluded_content: string[];
  deleted_content_history: string[];
}
...
// Smart Cleanup API
export async function getSmartCleanupConfig(): Promise<SmartCleanupConfig> {
  return invoke<SmartCleanupConfig>('get_smart_cleanup_config');
}

export async function saveSmartCleanupConfig(config: SmartCleanupConfig): Promise<void> {
  await invoke('save_smart_cleanup_config', { config });
}

export async function checkSmartCleanup(): Promise<TextClip[]> {
  return invoke<TextClip[]>('check_smart_cleanup');
}

export async function addDeletedContentToCleanup(content: string): Promise<void> {
  await invoke('add_deleted_content_to_cleanup', { content });
}
